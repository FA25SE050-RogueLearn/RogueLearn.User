...
public class RevokeGuildRoleCommandHandler : IRequestHandler<RevokeGuildRoleCommand, Unit>
{
    ...
    public RevokeGuildRoleCommandHandler(IGuildMemberRepository guildMemberRepository, ILogger<RevokeGuildRoleCommandHandler> logger, IGuildNotificationService notificationService)
    {
        _guildMemberRepository = guildMemberRepository;
        _logger = logger;
    ...
    public async Task<Unit> Handle(RevokeGuildRoleCommand request, CancellationToken cancellationToken)
    {
        ...
        {
            await _notificationService.NotifyRoleRevokedAsync(request.GuildId, request.MemberAuthUserId, oldRole, cancellationToken);
        }
