...
public class GetCurriculumProgramDetailsQueryHandler : IRequestHandler<GetCurriculumProgramDetailsQuery, CurriculumProgramDetailsResponse>
{
    ...
    {
        // Get the curriculum program
        var program = await _curriculumProgramRepository.GetByIdAsync(request.ProgramId, cancellationToken);
        if (program == null)
        {
            throw new NotFoundException("CurriculumProgram", request.ProgramId);
        }
        ...
        var curriculumVersions = await _curriculumVersionRepository.FindAsync(
            cv => cv.ProgramId == request.ProgramId,
            cancellationToken);
