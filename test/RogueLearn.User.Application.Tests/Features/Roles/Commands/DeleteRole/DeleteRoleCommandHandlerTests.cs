...
public class DeleteRoleCommandHandlerTests
{
    ...
    [Fact]
    public async Task Handle_AssignedToUsers_Throws()
    {
        var roleRepo = Substitute.For<IRoleRepository>();
        var userRoleRepo = Substitute.For<IUserRoleRepository>();
        var logger = Substitute.For<ILogger<DeleteRoleCommandHandler>>();
        ...
        var cmd = new DeleteRoleCommand { Id = roleId };
        var role = new Role { Id = roleId, Name = "r" };
        roleRepo.GetByIdAsync(roleId, Arg.Any<CancellationToken>()).Returns(role);
        userRoleRepo.GetUsersByRoleIdAsync(roleId, Arg.Any<CancellationToken>()).Returns(new List<UserRole> { new UserRole { RoleId = roleId, AuthUserId = Guid.NewGuid() } });
    ...
    public async Task Handle_Success_Deletes()
    {
        ...
        var role = new Role { Id = roleId, Name = "r" };
        roleRepo.GetByIdAsync(roleId, Arg.Any<CancellationToken>()).Returns(role);
        userRoleRepo.GetUsersByRoleIdAsync(roleId, Arg.Any<CancellationToken>()).Returns(new List<UserRole>());

        var sut = new DeleteRoleCommandHandler(roleRepo, userRoleRepo, logger);
        await sut.Handle(cmd, CancellationToken.None);
        await roleRepo.Received(1).DeleteAsync(roleId, Arg.Any<CancellationToken>());
    }
}