...
public class DeclineGuildJoinRequestCommandHandlerTests
{
    [Theory]
    [AutoData]
    public async Task Handle_GuildNotFound_Throws(DeclineGuildJoinRequestCommand cmd)
    {
        ...
        var sut = new DeclineGuildJoinRequestCommandHandler(guildRepo, reqRepo);
        await Assert.ThrowsAsync<RogueLearn.User.Application.Exceptions.NotFoundException>(() => sut.Handle(cmd, CancellationToken.None));
    }

    [Theory]
    [AutoData]
    public async Task Handle_RequestNotFound_Throws(DeclineGuildJoinRequestCommand cmd)
    {
        var guildRepo = Substitute.For<IGuildRepository>();
        var reqRepo = Substitute.For<IGuildJoinRequestRepository>();
    ...
    [Theory]
    [AutoData]
    public async Task Handle_RequestBelongsToDifferentGuild_Throws(DeclineGuildJoinRequestCommand cmd)
    {
        var guildRepo = Substitute.For<IGuildRepository>();
        var reqRepo = Substitute.For<IGuildJoinRequestRepository>();
        guildRepo.GetByIdAsync(cmd.GuildId, Arg.Any<CancellationToken>()).Returns(new Guild { Id = cmd.GuildId });
