...
public class JoinPublicPartyCommandHandlerTests
{
    ...
    [Fact]
    public async Task Handle_NewMember_DeclinesPendingInvitations()
    {
        var partyId = System.Guid.NewGuid();
        var authId = System.Guid.NewGuid();
        var cmd = new JoinPublicPartyCommand(partyId, authId);
        ...
        partyRepo.GetByIdAsync(partyId, Arg.Any<CancellationToken>()).Returns(new Party { Id = partyId, IsPublic = true, MaxMembers = 3 });
        memberRepo.CountActiveMembersAsync(partyId, Arg.Any<CancellationToken>()).Returns(0);
        memberRepo.GetMemberAsync(partyId, authId, Arg.Any<CancellationToken>()).Returns((PartyMember?)null);

        var pendingForThisParty = new PartyInvitation { Id = System.Guid.NewGuid(), PartyId = partyId, InviteeId = authId, Status = InvitationStatus.Pending };
        var alreadyDeclined = new PartyInvitation { Id = System.Guid.NewGuid(), PartyId = partyId, InviteeId = authId, Status = InvitationStatus.Declined };
        var otherPartyPending = new PartyInvitation { Id = System.Guid.NewGuid(), PartyId = System.Guid.NewGuid(), InviteeId = authId, Status = InvitationStatus.Pending };
        invRepo.GetPendingInvitationsByInviteeAsync(authId, Arg.Any<CancellationToken>())
            .Returns(new List<PartyInvitation> { pendingForThisParty, alreadyDeclined, otherPartyPending });

        await sut.Handle(cmd, CancellationToken.None);

        await invRepo.Received(1).UpdateRangeAsync(
            Arg.Is<IEnumerable<PartyInvitation>>(list =>
                list.All(i => i.PartyId == partyId && i.Status == InvitationStatus.Declined && i.RespondedAt.HasValue)
            ),
    ...
    public async Task Handle_ExistingInactiveMember_ActivatesAndDeclinesInvites()
    {
        ...
        await invRepo.Received(1).DeleteRangeAsync(Arg.Any<IEnumerable<System.Guid>>(), Arg.Any<CancellationToken>());
    }
}
